...
#define TOPIC_INLET_FLOW_LIVE   "home/waterflow/inlet/live_flow"
...
// -------------------- Heartbeat Watchdog --------------------
// Tracks when we last received a heartbeat from the outlet ESP32.
// If silent for >5 seconds, we suppress leak detection to avoid
// falsely closing the valve when the outlet device is offline.
unsigned long lastHeartbeatMillis = 0;
const unsigned long HEARTBEAT_TIMEOUT_MS = 5000;  // 5 seconds

// -------------------- Leak Thresholds (tuned for 1/2" x 33" pipe) --------------------
// Short pipe = near-zero propagation delay, so tight thresholds are safe.
// Sensor jitter on YF-S201 style sensors is ~±0.3 L/min, so 0.8 gives headroom.
const float leakThreshold    = 0.8;   // Flag leak if inlet exceeds outlet by this much (L/min)
const float restoreThreshold = 0.2;   // Clear leak when difference drops below this (L/min)
...
void mqttCallback(char* topic, byte* payload, unsigned int length) {
  ...
  if (String(topic) == TOPIC_VALVE_COMMAND) {
    if (message == "open") {
      ...
      // Clear any existing leak alert when user manually opens the valve
  ...
  // ── Outlet flow data from the second ESP32 ──────────────────────────────
  if (String(topic) == TOPIC_OUTLET_FLOW) {
    flowRateOutlet = message.toFloat();
  }
  if (String(topic) == TOPIC_OUTLET_TOTAL) {
    totalOutletML = message.toFloat() * 1000;  // convert L → mL
  }

  // ── Heartbeat from outlet ESP32 ─────────────────────────────────────────
  // Any message on this topic counts as proof the outlet device is alive.
...
void reconnectMQTT() {
  while (!client.connected()) {
    ...
    if (client.connect(clientId.c_str(), mqtt_username, mqtt_password)) {
      ...
      // Subscribe to valve commands from Flutter app
      ...
      // Subscribe to outlet heartbeat for watchdog
      client.subscribe(TOPIC_OUTLET_HEARTBEAT);

      // Publish initial valve status so Flutter app reflects correct state on startup
      client.publish(TOPIC_VALVE_STATUS, "Opened", true);
...
void setup() {
  ...
  digitalWrite(RELAY_PIN, HIGH);  // Valve initially open
  ...
  // Initialise heartbeat timer so the watchdog doesn't trigger immediately
  lastHeartbeatMillis = millis();
...
void loop() {
  ...
  // ── Inlet flow calculation every second ──────────────────────────────────
  if (currentMillis - previousMillis > interval) {
    ...
    // ── Heartbeat watchdog ──────────────────────────────────────────────────
    // If the outlet ESP32 has gone silent, suppress leak detection so we
    // don't falsely close the valve due to stale (zero) outlet readings.
    bool outletAlive = (millis() - lastHeartbeatMillis) < HEARTBEAT_TIMEOUT_MS;

    // ── Leak detection (only when outlet device is confirmed online) ────────
    if (outletAlive) {
      float flowDiff = flowRateInlet - flowRateOutlet;

      if (!leakDetected && flowDiff > leakThreshold) {
        // Increment confirmation counter — requires N consecutive seconds
        leakConfirmCount++;
      ...
      else if (leakDetected && flowDiff < restoreThreshold) {
        // Flow difference normalised — safe to reopen
        leakDetected     = false;
        leakConfirmCount = 0;
        openValve();
        client.publish(TOPIC_LEAK_ALERT,   "Normal", true);
        client.publish(TOPIC_VALVE_STATUS, "Opened", true);
        Serial.println("✅ Flow normalised — Valve reopened.");
      }
      else if (!leakDetected) {
        // No leak suspected this second — reset the confirmation counter
        leakConfirmCount = 0;
      }
    } else {
      // Outlet device offline — skip leak detection, reset counter
      leakConfirmCount = 0;
      Serial.println("⚠️  Outlet heartbeat timeout — leak detection suppressed.");
    }

    // ── Publish inlet data to MQTT ──────────────────────────────────────────
    char flowRateStr[10];
    char totalLStr[10];
    ...
    client.publish(TOPIC_INLET_FLOW,      flowRateStr);   // for leakages page
    client.publish(TOPIC_INLET_FLOW_LIVE, flowRateStr);   // dedicated live topic
...
void openValve() {
  valveState = true;
  digitalWrite(RELAY_PIN, HIGH);     // Energise relay → valve opens
...
void closeValve() {
  valveState = false;
  digitalWrite(RELAY_PIN, LOW);    // De-energise relay → valve closes
