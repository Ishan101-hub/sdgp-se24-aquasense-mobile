...
byte pulse1Sec = 0;
float flowRateOutlet = 0.0;                 // renamed for clarity
unsigned int flowMilliLitres = 0;
unsigned long totalMilliLitres = 0;

// Optional moving average buffer
#define MA_SIZE 3
float flowBuffer[MA_SIZE] = {0};
int bufferIndex = 0;
...
void loop() {
  ...
  if (currentMillis - previousFlowMillis > flowInterval) {
    ...
    flowRateOutlet = ((1000.0 / flowInterval) * pulse1Sec) / calibrationFactor;  
    flowMilliLitres = (flowRateOutlet / 60.0) * 1000.0;
    ...
    char flowRateStr[10];
    dtostrf(avgFlow, 4, 1, flowRateStr);
