...
class _UsageScreenState extends State<UsageScreen> {
  // ── Date selection ───────────────────────────────────────────────
  // Defaults to the PREVIOUS calendar month — this screen is a historical
  // report view; data for the current month is not yet finalised.
  late int selectedYear;
  late int selectedMonth;
  ...
  // Only years that have at least one selectable (past) month.
  List<String> get _selectableYears {
    final now = DateTime.now();
    // The earliest year we hold data for; extend left as needed.
    const firstYear = 2024;
    // If we're in January, the previous month is in last year, so include it.
  ...
  // Past months available for the currently selected year.
  List<String> get _selectableMonths {
    final now = DateTime.now();
    if (selectedYear < now.year) {
      // Any past year → all 12 months are fully closed.
      return _allMonths;
    }
    // Current year → only months strictly before this month are closed.
    return _allMonths.sublist(0, now.month - 1);
  }
  ...
  void initState() {
    super.initState();
    // Default: previous month (handles January → December of last year).
    final now = DateTime.now();
    if (now.month == 1) {
  ...
  Future<void> _loadSummary() async {
    ...
    try {
      summary = await _api.fetchUsageSummary(
        year:  selectedYear,
        month: selectedMonth,
      );
    } catch (e) {
  ...
  void _onYearChanged(String val) {
    ...
    setState(() {
      ...
      // If the previously selected month is no longer valid (e.g. user switches
      // to the current year and selectedMonth >= now.month), clamp it.
      final now = DateTime.now();
      if (newYear == now.year && selectedMonth >= now.month) {
        selectedMonth = now.month - 1 == 0 ? 12 : now.month - 1;
      }
    });
    _loadSummary();
  }
  ...
  void _onMonthChanged(String val) {
    setState(() => selectedMonth = _allMonths.indexOf(val) + 1);
    _loadSummary();
  }
  ...
  // ── Computed display values (always use final monthly_total) ─────
  String get _monthly  => _summary != null ? _fmt(_summary!.monthlyTotal) : '--';
  String get _weekly   => _summary != null ? _fmt(_summary!.weeklyAvg)    : '--';
  String get _daily    => _summary != null ? _fmt(_summary!.dailyAvg)     : '--';
  String get _leaks    => _summary != null ? '${_summary!.leakCount}'     : '--';

  String get _selectedMonthLabel =>
      '${_allMonths[selectedMonth - 1]} $selectedYear';
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            children: [
              ...
              Center(
                child: _buildInlineDateSelector(
                  aquaAccent, scale, theme, isDark,
                ),
              ),
              SizedBox(height: 6 * scale),

              // ── "Data retrieved at end of month" caption ─────
              Text(
                'Showing finalised data for $_selectedMonthLabel',
                style: TextStyle(
                  fontSize: 11 * scale,
              ...
              if (_isLoading)
                ...
                _buildMainUsageCard(
                  mainTextColor, theme.cardColor, scale, theme, isDark,
                ),
                SizedBox(height: 15 * scale),
                Row(
                  children: [
                ...
                SizedBox(height: 20 * scale),
                _buildLeakCard(theme.cardColor, scale, theme, isDark),
                SizedBox(height: 25 * scale),
                _buildReportButton(aquaAccent, scale),
                SizedBox(height: 80 * scale),
              ],
  ...
  // ── Date selector ────────────────────────────────────────────────
  Widget _buildInlineDateSelector(
    Color color, double scale, ThemeData theme, bool isDark,
  ) {
    return Container(
      padding: EdgeInsets.symmetric(
        horizontal: 12 * scale,
          ...
          child: Text(
            ...
            style: TextStyle(
              color: item == currentVal
                  ...
                  : (isDark ? Colors.white : Colors.black87),
              fontWeight:
                  item == currentVal ? FontWeight.bold : FontWeight.normal,
            ),
          ),
        );
      }).toList(),
  ...
  // ── Cards ────────────────────────────────────────────────────────
  Widget _buildMainUsageCard(
    Color textColor, Color cardBg, double scale, ThemeData theme, bool isDark,
  ) {
    return Container(
      width: double.infinity,
      padding: EdgeInsets.symmetric(
      ...
      child: Column(
        children: [
          ...
          // Days-with-data footnote (informational only, no projection).
          if ((_summary?.daysWithData ?? 0) > 0) ...[
            SizedBox(height: 10 * scale),
            Text(
              'Across ${_summary!.daysWithData} of ${_summary!.daysInMonth} days',
              style: TextStyle(
                fontSize: 12 * scale,
                color: isDark ? Colors.white38 : Colors.grey[500],
              ),
  ...
  Widget _buildLeakCard(
    Color cardBg, double scale, ThemeData theme, bool isDark,
  ) {
    return Container(
      width: double.infinity,
      padding: EdgeInsets.all(22 * scale),
