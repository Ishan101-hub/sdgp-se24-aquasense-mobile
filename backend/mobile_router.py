...
# mobile_router.py
# AquaSense v3.2 — Mobile API Router
#
# All endpoints the Flutter app calls directly.
# Designed to match the exact JSON shapes the Flutter code expects.
#
...
# The Flutter app sends:  Authorization: Bearer <token>
#
# Changes v3.2 (leakages screen only — home page endpoints unchanged):
#   • leakages()             — valve_state now sourced from the INLET device,
#                              not the outlet device. The physical relay is
#                              wired to the inlet ESP32; the outlet ESP32 has
#                              no relay and its valve_state is never updated.
#                              Query 3 is outlet-flow-only; new Query 4 reads
#                              inlet Device.valve_state per zone.
#   • mobile_valve_command() — removed 409 / override pattern. When the user
#                              opens the valve while a leak alert is active it
#                              means they have physically resolved the leak.
#                              Active alerts are now auto-resolved and the
#                              in-memory LeakDetectionService state is cleared
#                              before the open command is sent to the ESP32.
...
# # ─────────────────────────────────────────────────────────────────────────────
# #  GET /mobile/flowrate   — UNCHANGED from v3.1
# # ─────────────────────────────────────────────────────────────────────────────
...
# # ─────────────────────────────────────────────────────────────────────────────
# #  GET /mobile/dashboard/today   — UNCHANGED from v3.1.1
# #
# #  FIX (v3.1.1): today's volume uses restart-safe consecutive-delta odometer.
# # ─────────────────────────────────────────────────────────────────────────────
...
# #  GET /mobile/leakages
# #
# #  v3.2 FIX: valve_state now sourced from the INLET device per zone.
# #
# #  REASON: The physical solenoid valve relay is wired to the INLET ESP32
# #  (pipe_01_inlet). The outlet ESP32 (pipe_01_outlet) is a pure flow-meter
# #  with no relay — its Device.valve_state field is never updated by valve
# #  commands or ESP32 acks and will always show a stale/wrong value.
# #
# #  The fix splits the old Query 3 into two:
# #    Query 3 — outlet flow_rate only (no valve_state)
# #    Query 4 — inlet Device.valve_state per zone (source of truth)
# #
# #  FIX (v3.1.1): replaced N*2 per-zone DB queries with bulk queries +
...
#     # ── Query 3: latest outlet flow_rate per zone (no valve_state) ────────
#     # The outlet device is a pure flow-meter — it has no relay and its
#     # valve_state column is never updated by valve commands or ESP32 acks.
#     outlet_sub = (
#         select(
#             Device.zone_id,
#             func.max(Reading.timestamp).label("latest_ts"),
...
# #  POST /mobile/valve
# #
# #  v3.2 FIX: removed 409 / override pattern.
# #
# #  OLD BEHAVIOUR: action=open while alerts are active → 409, required
# #  override=true. User had to resolve the alert separately first.
# #
# #  NEW BEHAVIOUR: action=open auto-resolves all active leak/valve-failure
# #  alerts for the zone and clears the in-memory LeakDetectionService state,
# #  then proceeds to open the valve. Toggling the switch open IS the user's
# #  acknowledgement that the leak has been physically repaired.
# #
# #  The override field is kept in the request body for client compatibility
# #  but is no longer read by the server.
# # ─────────────────────────────────────────────────────────────────────────────
...
#         # Clear in-memory state so the detection counter resets cleanly.
#         # The 10-second settle window starts automatically when the next
#         # inlet reading arrives after the valve is opened.
#         from leak_service import get_leak_service
#         svc = get_leak_service()
#         if svc:
...
# mobile_router.py
# AquaSense v3.3 — Mobile API Router
#
# All endpoints the Flutter app calls directly.
#
# Changes v3.3:
