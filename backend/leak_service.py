...
Scenario 3 — settle window:
  After the valve transitions from closed → open (user re-opens after a leak
  OR after any manual close/open cycle), leak detection is suppressed for
  SETTLE_WINDOW_SECONDS so turbulent flow at valve-open time does not
  immediately re-trigger a false alarm.
  This mirrors the ESP32 isSettling / SETTLE_DURATION_MS logic exactly.

v3.4 addition — force_settle():
  ROOT CAUSE of false positives:
    _process_inlet_reading() passes device.valve_state from the in-process
    device cache (TTL = 60 s) to update_valve_state(). If the user manually
    closes then re-opens the valve, the stale cache keeps feeding "open" to
    update_valve_state() even while the valve is physically closed. By the
    time the user re-opens, the LeakDetectionService already thinks the valve
    is OPEN. _process_valve_status("Opened") then finds no state transition
    (OPEN → OPEN), so NO settle window is started. Inlet flow rises
    immediately, outlet lags, delta > threshold → false leak in 3 seconds.

  FIX:
    Added force_settle(zone_id) — called directly by mobile_valve_command
    when action="open", BEFORE the MQTT command even reaches the ESP32.
    It unconditionally sets valve_state=OPEN and starts the settle window,
    bypassing the "transition required" guard in update_valve_state().
    The companion fix in mobile_valve_command also calls
    invalidate_device_cache() for every inlet device so the stale-cache
    mechanism that caused the phantom OPEN→OPEN state is cleared.
"""
...
# How long (seconds) to suppress leak detection after the valve is opened.
# Must match ESP32: SETTLE_DURATION_MS = 10 000 ms.
SETTLE_WINDOW_SECONDS: int = 10
...
class ZoneState:
    ...
    # Settle window: set to a future datetime when the valve transitions to OPEN.
    # Leak detection is suppressed until this time passes.
    # Matches ESP32: isSettling / settleStartMillis / SETTLE_DURATION_MS.
    settle_until: Optional[datetime] = None
...
class LeakDetectionService:
    ...
    async def update_valve_state(self, zone_id: int, valve_state: ValveState) -> None:
        ...
        Called by:
          • _process_inlet_reading  (from cached device — may be stale)
          • _process_valve_status   (direct ESP32 ACK — always accurate)
          • _process_event_message  (ESP32 leak alert with valve field)

        The settle window is started on a CLOSED/UNKNOWN → OPEN transition.
        If the LeakDetectionService already has valve_state=OPEN (because the
        stale device cache kept feeding OPEN while the valve was physically
        closed), this method will NOT start a new settle window on re-open
        because there is no transition.  Use force_settle() when the user
        explicitly opens the valve from the app to guarantee the settle window
        regardless of current in-memory state.
        """
        state = self._get_zone_state(zone_id)
        if state.valve_state != valve_state:
            logger.info(
            ...
            if valve_state == ValveState.OPEN:
                ...
                # Start settle window — matches ESP32 isSettling guard
                state.settle_until = datetime.utcnow() + timedelta(seconds=SETTLE_WINDOW_SECONDS)
                logger.info(
                    "Zone %d: valve opened — settle window started (%ds)",
                    zone_id, SETTLE_WINDOW_SECONDS,
    ...
    def force_settle(self, zone_id: int) -> None:
        """
        Unconditionally start the settle window for a zone.

        WHY THIS IS NEEDED:
        update_valve_state() only starts the settle window on a state
        TRANSITION (e.g. CLOSED → OPEN). If the device cache is stale and
        has been feeding OPEN to update_valve_state() while the valve was
        physically closed, the in-memory valve_state is already OPEN. When
        the user re-opens the valve, _process_valve_status("Opened") calls
        update_valve_state(OPEN) but finds no transition → no settle window
        → inlet surges, outlet lags → false leak in 3 seconds.

        This method bypasses the transition guard and unconditionally:
          • Sets valve_state = OPEN (source of truth for this zone)
          • Resets the mismatch counter to 0
          • Clears all active alert flags
          • Sets settle_until = now + SETTLE_WINDOW_SECONDS

        It is called by mobile_valve_command immediately when action="open",
        BEFORE the MQTT "open" command is even sent to the ESP32. This
        guarantees the server backup detector is suppressed for the full
        settle window regardless of what state the service was in before.

        Matches the ESP32 behaviour:
          mqttCallback: if (cmd == "open") {
            leakDetected = false;
            leakConfirmCount = 0;
            openValve();
            isSettling = true;         ← always set, no transition guard
        ...
        state.is_leak_active          = False
        state.is_valve_failure_active = False
        state.settle_until            = now + timedelta(seconds=SETTLE_WINDOW_SECONDS)

        logger.info(
            "Zone %d: FORCE settle window started (%ds) — valve opened by user",
            zone_id, SETTLE_WINDOW_SECONDS,
        )
    ...
    async def _evaluate_leak_condition(self, zone_id: int) -> None:
        ...
        # STEP 3 — timestamp alignment (server-only stale-delta guard)
        # If inlet and outlet readings are more than 2 seconds apart, the
        # delta is stale and unreliable. Reset counter and wait for fresh pair.
        if state.inlet_timestamp and state.outlet_timestamp:
            ts_gap = abs(
                (state.inlet_timestamp - state.outlet_timestamp).total_seconds()
            )
    ...
    async def clear_leak(self, zone_id: int) -> None:
        ...
        Clear leak flag when user resolves an alert.
        Valve stays closed — user must open it explicitly via the app toggle.
        A settle window is NOT started here — it is started by force_settle()
        which is called separately when the valve is actually opened.
        """
        state = self._get_zone_state(zone_id)
        if not state.is_leak_active:
            return
