...
#         #
#         # Normalise legacy DB value "close" → "closed" before constructing the
#         # enum. Older firmware/DB rows stored "close" instead of "closed";
#         # ValveState only accepts "open" / "closed", so raw construction raises
#         # ValueError("'close' is not a valid ValveState").
#         _raw_vs = (device.valve_state or "open").lower()
#         _vs_enum = ValveState.CLOSED if "close" in _raw_vs else ValveState.OPEN
...
#         # Sensor readings are high-frequency — debug only to avoid log spam
#         logger.debug("SENSOR %s/%s device=%s value=%s", location, msg_type, parsed["device_id"], raw_text)
...
#                 logger.info("LEAK CLEAR device=%s", parsed["device_id"])
#                 return

#             logger.info("LEAK ALERT device=%s payload=%s", parsed["device_id"], raw_text[:120])
#             try:
...
#             logger.info("VALVE STATUS device=%s payload=%s", parsed["device_id"], raw_text)
...
#                     asyncio.create_task(
#                         _dispatch(topic, message.payload)
#                     )
...
#                     if "valve" in topic or "leak" in topic:
...
# Supabase PgBouncer in Session mode hard-caps simultaneous client connections
# to its pool_size. Without a gate here, MQTT bursts spawn N concurrent tasks
# that all try to open a DB session at once, exceeding that cap and triggering
# "MaxClientsInSessionMode: max clients reached".
#
# This semaphore ensures at most 3 tasks hold a DB session simultaneously —
# matching the pool_size=3 set in database.py. Excess tasks wait here instead
# of racing to create connections PgBouncer will reject.
...
async def _dispatch(topic: str, raw_payload: bytes) -> None:
    ...
    # ── Leak alert ────────────────────────────────────────────────────────
        ...
        if msg_type == "alert":
            ...
            # Handle JSON payload — parse first, then check status before any DB work.
            # ESP32 sends {"status":"NORMAL","valve":"OPEN"} as a retained message on
            # every reconnect. Without this guard, "NORMAL" falls through to
            # _process_event_message, hits the DB unnecessarily, and can trigger
            # the MaxClientsInSessionMode crash + MQTT disconnect loop.
            try:
                payload = json.loads(raw_text)
            except json.JSONDecodeError:
                logger.warning("Non-JSON leak payload on %s: %r", topic, raw_text[:120])
            ...
            if str(payload.get("status", "")).upper() == "NORMAL":
                logger.info("LEAK CLEAR (JSON) device=%s", parsed["device_id"])
                return
...
async def start_mqtt_listener(publish_queue: asyncio.Queue) -> None:
    ...
    # Append a short unique suffix so two running instances (e.g. local + Render)
    # never share the same client ID. HiveMQ kicks out whichever connected first
    # when a second client with the identical ID arrives — causing the 5-second
    # disconnect loop seen in the logs.
    session_id = uuid.uuid4().hex[:8]
    client_id  = f"{settings.MQTT_CLIENT_ID}-{session_id}"
    logger.info("MQTT client ID for this session: %s", client_id)

    backoff = 5  # seconds; doubles on each failed attempt, capped at 60 s
    ...
    while True:
        ...
        try:
            async with aiomqtt.Client(
                ...
                password=settings.MQTT_PASSWORD,
                tls_params=aiomqtt.TLSParameters(),
                keepalive=30,   # Send PINGREQ every 30 s — broker drops at 1.5× = 45 s idle
            ) as client:
            ...
            logger.error("MQTT error: %s — retrying in %ds", exc, backoff)
            await asyncio.sleep(backoff)
            backoff = min(backoff * 2, 60)  # Exponential backoff, max 60 s